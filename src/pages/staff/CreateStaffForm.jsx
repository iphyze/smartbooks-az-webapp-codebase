...
const CreateStaffForm = () => {
  ...
  const [staffDetails, setStaffDetails] = useState({
    // staff_id is removed from manual input state
    staff_name: "",
    staff_email: "",
    staff_tel: "",
    staff_address: "",
    date_of_birth: null, // Initialized as null for DatePicker
    gender: "",
    job_title: "",
    date_of_joining: new Date(), // Default to today
    bank_name: "",
    bank_account_number: "",
    bank_account_name: "",
    pension_number: "",
  ...
  const validateForm = useCallback(() => {
    const e = {};
    // staff_id validation is handled by the auto-generation logic check before submit
    ...
    if (!staffDetails.generate_staff)
      e.generate_staff = "Ledger option is required";
    return e;
  }, [staffDetails]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!nextStaffId) {
      showToast("Staff ID is still loading, please wait", "error");
      return;
    }
    ...
    const payload = {
      staff_id: nextStaffId, // Use auto-generated ID
      ...staffDetails,
      ...
      >
        ...
        >
          ...
          <div className="invoice-form-header">
            <div className="invoice-form-htxt">Add New Staff Member</div>
            <div className="invoice-form-sub-htxt">
              Fill in the details below to create a staff record
            </div>
          </div>
            ...
            <div className="invoice-form invoice-form-three">
              <div className="input-form-wrapper">
                <div className={`input-form-group ${errors.generate_staff ? "input-form-error" : ""}`}>
                  <label className={`input-form-label ${errors.generate_staff ? "input-label-message" : ""}`} htmlFor="generate_staff">
                    Generate Payroll Ledger?
                  </label>
                  <div className="form-wrapper">
                    <Select
                      ...
                      classNamePrefix="form-input-select"
                      inputId="generate_staff"
                      onMenuOpen={() => setOpenMenuId("generate_staff")}
                      onMenuClose={() => setOpenMenuId(null)}
                    />
                    <span className={["chevron-input-icon fas fa-chevron-down", openMenuId === "generate_staff" ? "chevron-rotate" : "", errors.generate_staff ? "input-icon-error" : ""].filter(Boolean).join(" ")} />
                  </div>
                </div>
                {errors.generate_staff && (
                  <div className="input-error-message">{errors.generate_staff}</div>
                )}
              </div>
            </div>
          ...
          <div className="invoice-action-btn main-submit-action-btn">
            <div className="invoice-action-btn-wrapper">
              <button
                type="submit"
                disabled={isLoading || fetchingStaffId}
                className="invoice-submit-btn"
              >
                {isLoading ? (
                  <div className="invoice-loader" />
                ) : (
                  <span className="invoice-submit-btn-text">Add Staff</span>
                )}
              </button>
            </div>
          </div>
