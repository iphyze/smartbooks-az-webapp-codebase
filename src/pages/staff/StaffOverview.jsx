...
const StaffOverview = () => {
  ...
  const links = [
    ...
    { label: "Staff", to: "/staff/home", active: false }
  ];
  ...
  const handleSelectAll = () => {
    const currentPageIds = data.map(item => item.staff_id);
    ...
    if (allSelected) {
      ...
      useStaffStore.setState({ selectedItems: newSelection });
    } else {
  ...
  // Handler for single row delete click
  ...
  const handleCloseErrorModal = () => {
    useStaffStore.setState({ error: null });
  };

  const handleViewStaff = (staff) => {
    navigate(`/staff/view/${staff.staff_id}`, { state: { staff } });
  };

  const handleEditStaff = (staff) => {
    navigate(`/staff/edit/${staff.staff_id}`, { state: { staff } });
  };
      ...
      <div className={`content-container theme-${theme}`}>
        ...
        <div className={`db-root theme-${theme}`}>
          <div className="db-page">
            <PageNav pageTitle='Staff Overview' links={links} />
            ...
            >
              <div className="top-action-wrapper">
                <Link to='/staff/create-staff' className="create-new-invoice-btn">
                ...
                {loading ? (
                  ...
                  <>
                        ...
                        <table className="data-table invoice-table">
                          <thead>
                            ...
                            ${selectedItems.length === data.length && data.length > 0 && 'selected-checkbox'}`}
                              ...
                              <th onClick={() => handleSort('date_of_joining')} className="sortable">
                                Date Joined {getSortIcon('date_of_joining')}
                              </th>
                              <th>Actions</th>
                            </tr>
                          </thead>
                          <tbody>
                            {data.map((staff) => (
                              <tr key={staff.staff_id} className={selectedItems.includes(staff.staff_id) ? 'selected' : ''}>
                                <td className="checkbox-cell">
                                  <input
                                    type="checkbox"
                                    className={`table-checkbox fas fa-check ${selectedItems.includes(staff.staff_id) && 'selected-checkbox'}`}
                                    checked={selectedItems.includes(staff.staff_id)}
                                    onChange={() => toggleItemSelection(staff.staff_id)}
                                  />
                                </td>
                                <td className="number-tab">{staff.staff_id}</td>
                                <td>
                                  <div className="table-flex-box">
                                    <span className="table-customer-text">{staff.staff_name}</span>
                                  </div>
                                </td>
                                <td>{staff.job_title || '—'}</td>
                                <td>{staff.staff_email}</td>
                                <td className="number-tab">{staff.staff_tel || '—'}</td>
                                <td className="number-tab">{formatDate(staff.date_of_joining)}</td>
                                <td>
                                  <div className="action-buttons">
                                    <button className="btn-view" title="View" onClick={() => handleViewStaff(staff)}>
                                      <span className="fas fa-file"></span>
                                    </button>
                                    <button className="btn-edit" title="Edit" onClick={() => handleEditStaff(staff)}>
                                      <span className="fas fa-pen"></span>
                                    </button>
                                    {/* Added Delete Button */}
                                    <button className="btns-delete" title="Delete" onClick={() => handleDeleteStaff(staff.staff_id)}>
                    ...
                    {data.length === 0 && (
                      <EmptyTable
                        icon="fas fa-users"
                        message="No staff records found matching your criteria"
                        link="/staff/create-staff"
                      />
                    )}
                  </>
                )}
