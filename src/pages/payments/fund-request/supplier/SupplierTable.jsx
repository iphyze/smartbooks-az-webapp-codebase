...
import useSupplierRequestStore from "../../../../stores/fund-request/useSupplierRequestStore";
import UpdateModal from "../../../../components/UpdateModal";
import PrepareSupplierGapsModal from "./PrepareSupplierGapsModal";
...
const SupplierTable = () => {
  ...
  const {
    ...
    getTotalPages, exportToExcel, getCurrentPageData, clearSelection,
    setPaymentStatus, payment_status, loading, year, setYear
  } = useSupplierRequestStore();
...
const handleYearChange = (e) => {
  ...
  if (e.target.checked) {
    ...
    useSupplierRequestStore.setState({ 
  ...
  useEffect(() => {
    ...
    document.title = "Acctlab | Supplier Fund Request";
    fetchData();
  ...
  const getInitials = (name) => {
    ...
    const truncateWithTooltip = (text, limit = 20) => {
      if (!text) return '';
      const isTruncated = text.length > limit;
      return {
        displayText: isTruncated ? `${text.substring(0, limit)}...` : text,
        tooltipText: text
      };
    };
  ...
  return(
    ...
    <div className={`main-container theme-${theme}`}>
      ...
      <div className={`content-container theme-${theme}`}>
        ...
        <div className="data-table-title">Supplier</div>
        ...
        <div className="filters-user-section">
            ...
            {selectedItems.length > 0 && (
                <button onClick={clearSelection} className="clear-selection-button">
                    <span className="fas fa-times"></span> Clear Selection
                </button>
            )}
              ...
              <table className="datatable-table">
                <thead>
                  <tr>
                    ...
                    <th onClick={() => handleSort('invoice_number')} className="sortable-header">
                      Invoice Details
                      <span className="sort-icon">
                    ...
                    <th onClick={() => handleSort('net_value')} className="sortable-header">
                      Net Value
                      <span className="sort-icon">
                        {sortBy === 'net_value' ? (
                          <i className={`fas fa-sort-${sortOrder === 'ASC' ? 'up' : 'down'}`} />
                ...
                <tbody>
                  {currentItems.map((item, index) => (
                    <tr key={item.id}>
                      ...
                      <td title={truncateWithTooltip(item.invoice_number).tooltipText}>
                        <b>Inv. No.:</b> {item.invoice_number}, <b>Pur. No.:</b> {item.purchase_number}, <br /><br />
                        <b>Po. No.:</b> {item.po_number},
                      </td>
                      <td><div className="tab-box-bankname">₦ {formatAmount(item.net_value)}</div></td>
                      <td><div className="tab-box-bankname">₦ {formatAmount(item.amount)}</div></td>
