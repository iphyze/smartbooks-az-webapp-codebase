...
const PrepareSupplierGapsModal = ({ isOpen, onClose, selectedItems, data, onSuccess }) => {
  const [selectedDate, setSelectedDate] = useState(null);
  const [selectedBatch, setSelectedBatch] = useState('');
  const [selectedAccount, setSelectedAccount] = useState(null);
  const [remark, setRemark] = useState(`Payment against Inv. No. ${data.map(item => item.invoice_number).join(', ')}, Po No. ${data.map(item => item.po_number).join(', ')}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.post('/gaps/supplier/suppliersGaps', gapsData, {
...
export default PrepareSupplierGapsModal;