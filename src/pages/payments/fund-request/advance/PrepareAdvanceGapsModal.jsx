...
const PrepareAdvanceGapsModal = ({ isOpen, onClose, selectedItems, data, onSuccess }) => {
  const [selectedDate, setSelectedDate] = useState(null);
  const [selectedBatch, setSelectedBatch] = useState('');
  const [selectedAccount, setSelectedAccount] = useState(null);
  const [remark, setRemark] = useState(`100% Payment against Po No. ${data.map(item => item.po_number).join(', ')}`);
  ...
  const totalAmount = useMemo(() => {
    return data.reduce((sum, item) => sum + parseFloat(item.advance_payment), 0);
  }, [data]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const gapsData = [{
        ...
        batch: selectedBatch,
        remark: remark,
        percentages: data.map(item => item.percentage).join(', '),
        po_numbers: data.map(item => item.po_number).join(', '),
      ...
      await api.post('/gaps/advance/suppliersAdvanceGaps', gapsData, {
...
export default PrepareAdvanceGapsModal;