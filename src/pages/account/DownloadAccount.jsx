...
const DownloadAccount = ({ account, ledgers = [], accountSummary = {} }) => {
    // Destructure account properties
    const {
        type, category_id, category, sub_category,
    ...
    return (
        <Document>
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.headerContainer}>
                    <View style={styles.headerLeft}>
                        <View style={styles.metaRow}>
                            ...
                            <Text style={styles.metaValue}>{type || 'N/A'}</Text>
                        </View>
                        <View style={styles.metaRow}>
                            <Text style={styles.metaLabel}>Category ID:</Text>
                            <Text style={styles.metaValue}>{category_id || 'N/A'}</Text>
                        </View>
                        <View style={styles.metaRow}>
                            <Text style={styles.metaLabel}>Category:</Text>
                            <Text style={styles.metaValue}>{category || 'N/A'}</Text>
                        </View>
                        <View style={styles.metaRow}>
                            <Text style={styles.metaLabel}>Sub Category:</Text>
                            <Text style={styles.metaValue}>{sub_category || 'N/A'}</Text>
                        </View>
                        <View style={styles.metaRow}>
                    ...
                    <View style={styles.headerRight}>
                        <Text style={styles.headerTypeText}>CATEGORY ID #</Text>
                        <Text style={styles.headerId}>{category_id || 'N/A'}</Text>
                    </View>
                </View>
                            ...
                            {Object.entries(accountSummary).map(([currency, data]) => (
                                <View key={currency} style={styles.summaryCard}>
                                    <View style={styles.cardHeaderBg}>
                                        <Text style={styles.cardHeaderText}>{currency} Summary</Text>
                                    </View>
                                    <View style={styles.cardBody}>
                                        ...
                                        <View style={[styles.cardRow, styles.balanceRow]}>
                                            <Text style={[styles.cardLabel, styles.boldText]}>Balance</Text>
                                            <Text style={[styles.cardValue, styles.boldText]}>{formatCurrencyDecimals(data.balance || 0, currency)}</Text>
                                        </View>
                                    </View>
                                </View>
                            ))}
                ...
                {/* Ledgers Table Section */}
                <View style={styles.sectionHeader}>
                    <Text style={styles.sectionTitle}>Associated Ledgers ({ledgers.length})</Text>
                </View>
                ...
                {ledgers.length > 0 ? (
                    <View style={styles.table}>
                        <View style={[styles.tableRow, styles.tableHeader]}>
                            ...
                            <Text style={[styles.tableCell, styles.colNumber, styles.colColor]}>Ledger Number</Text>
                            <Text style={[styles.tableCell, styles.colSubClass, styles.colColor]}>Sub Class</Text>
                            <Text style={[styles.tableCell, styles.colType, styles.colColor]}>Type</Text>
                        </View>

                        {ledgers.map((ledger, index) => (
                            <View style={[styles.tableRow, index % 2 !== 0 && styles.tableRowOdd]} key={ledger.id || index}>
                                <Text style={[styles.tableCell, styles.colSn]}>{index + 1}</Text>
                                <Text style={[styles.tableCell, styles.colName]}>{ledger.ledger_name || 'N/A'}</Text>
                                <Text style={[styles.tableCell, styles.colNumber]}>{ledger.ledger_number || 'N/A'}</Text>
                                <Text style={[styles.tableCell, styles.colSubClass]}>{ledger.ledger_sub_class || 'N/A'}</Text>
                                <Text style={[styles.tableCell, styles.colType]}>{ledger.ledger_type || 'N/A'}</Text>
                            </View>
                        ))}
                    </View>
                ) : (
                    <View style={styles.emptyState}>
                        <Text style={styles.emptyStateText}>No ledgers have been created for this account type yet.</Text>
                    </View>
                )}
...
const styles = StyleSheet.create({
    ...
    colName: { flex: 0.38, textAlign: 'left' },
    colNumber: { flex: 0.15, textAlign: 'left' },
    colSubClass: { flex: 0.2, textAlign: 'left' },
    colType: { flex: 0.2, textAlign: 'left', borderRightWidth: 0 },
    boldText: {
