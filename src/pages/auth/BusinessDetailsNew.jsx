...
const BusinessDetails = () => {
  ...
  // Get personal info from navigation state
  ...
  const validationSchema = {
    businessName: [validationRules.required, validationRules.minLength(3)],
    businessType: [validationRules.required],
    businessSize: [validationRules.required],
    tin: [
      validationRules.required,
      // custom rule for TIN pattern
      (value) => {
        const isValid = /^[0-9-]+$/.test(String(value || ""));
        return { isValid, message: isValid ? "" : "TIN should contain only numbers and dashes" };
      }
    ],
    industry: [validationRules.required]
  };
  const { errors, setErrors, validateField, validateForm, handleBlur, touched, touchField } = useFormValidation(validationSchema, businessDetails);
  ...
  const handleChange = (field, value) => {
    ...
    touchField(field); // Mark the field as touched
    const errorMessage = validateField(field, value); // Validate the field
    setErrors(prev => ({ ...prev, [field]: errorMessage })); // Update errors
  };
          ...
          <motion.div variants={fadeInUp} initial="hidden" whileInView="show"
            ...
            <form onSubmit={handleContinue} className="login-form-f-container">
              ...
              <div className="login-form-box">
                <label className={`login-form-label ${touched.businessName && errors.businessName ? 'login-error-message' : ''}`}>Business Name</label>
                <input type="text" value={businessDetails.businessName} 
                onChange={(e) => handleChange("businessName", e.target.value)}
                onBlur={() => handleBlur('businessName')}
                  placeholder="Enter your business name" className={`login-form-input ${touched.businessName && errors.businessName ? 'border-error' : ''}`}
                />
                {touched.businessName && errors.businessName && <div className="login-error-message">{errors.businessName}</div>}
              </div>
              ...
              <LocalSearchableSelect
                ...
                onChange={(value) => handleChange("businessType", value)} // Ensure it calls handleChange
            ...
            />
              ...
              <div className="login-form-box login-form-half">
                ...
                <input
                  ...
                  onBlur={(e) => handleBlur("tin")}
                  placeholder="Enter your TIN (e.g., 12345678-0001)"
                  className={`login-form-input ${touched.tin && errors.tin ? 'border-error' : ''}`}
                />
                {touched.tin && errors.tin && <div className="login-error-message">{errors.tin}</div>}
              </div>
            ...
            <LocalSearchableSelect
                ...
                onChange={(value) => handleChange("industry", value)} // Ensure it calls handleChange
