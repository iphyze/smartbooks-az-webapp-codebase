...
import { customers } from "../invoice/data/customerData";
...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    const accounting_period = new Date().getFullYear();

    try {
      const result = await login(email, password, accounting_period);
      ...
      <div className="login-flexbox">
          ...
          <form onSubmit={handleLogin} className="login-form-container" data-aos='fade-up'>
            ...
            <div className="login-form-box">
              <div className={`login-form-group ${errors.email ? 'login-form-error' : ''}`}>
                <label className={`login-form-label ${errors.email ? 'login-label-message' : ''}`} htmlFor="email">Email</label>
                <div className="login-form-wrapper">
                  <input type="email" placeholder="Enter Email" value={email} onChange={(e) => setEmail(e.target.value)} id="email"
                  className={`login-input ${errors.email ? 'login-input-error' : ''}`}
                  />
                  <span className={`login-icon fas fa-envelope ${errors.email ? 'login-icon-error' : ''}`}></span>
                </div>
              </div>
              {errors.email && <div className="login-error-message" data-aos='fade-in'>{errors.email}</div>}
            </div>

            <div className="login-form-box">
              <div className={`login-form-group ${errors.password ? 'login-form-error' : ''}`}>
                <label className={`login-form-label ${errors.password ? 'login-label-message' : ''}`} htmlFor="password">Password</label>
                <div className="login-form-wrapper">
                  ...
                  />
                  <span className={`login-icon fas fa-lock ${errors.password ? 'login-icon-error' : ''}`}></span>
                  <button type="button" className={`login-show-btn ${errors.password ? 'login-icon-error' : ''}`} onClick={() => setShowPassword(!showPassword)}>{showPassword ? 'Hide' : 'Show'}</button>
                </div>
              </div>
              {errors.password && <div className="login-error-message" data-aos='fade-in'>{errors.password}</div>}
            </div>
