...
const useCompassRequestStore = create((set, get) => ({
  // Data states
  data: [],
  loading: false,
  error: null,
  ...
  // Meta states
  total: 0,
  year: localStorage.getItem('compass_selected_year') || useAuthStore.getState().user?.accounting_period || new Date().getFullYear(),
  payment_status: localStorage.getItem('compass_payment_status') || 'all',

  // Pagination states
  currentPage: 1,
  itemsPerPage: localStorage.getItem('compass_payment_limit') || 10,
  ...
  fetchData: async () => {
    ...
    try {
      ...
      const response = await api.get(`/request/compass/getFilteredRequest?${params}`, {
      ...
      localStorage.setItem('compass_selected_year', year);
      localStorage.setItem('compass_payment_status', payment_status);
      localStorage.setItem('compass_selected_limit', itemsPerPage);
    } catch (error) {
      set({
...
setSorting: (newSortBy, newSortOrder) => {
  ...
  setItemsPerPage: (items) => {
    ...
    localStorage.setItem('compass_selected_limit', items);
    get().fetchData();
  },
  ...
  setPaymentStatus: (status) => {
    ...
    localStorage.setItem('compass_payment_status', status);
  ...
  deleteSelectedItems: async () => {
    ...
    try {
      await api.delete('/request/compass/delete', {
        headers: { Authorization: `Bearer ${token}` },
...
export default useCompassRequestStore;
