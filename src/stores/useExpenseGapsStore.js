...
const useExpenseGapsStore = create((set, get) => ({
  // Data states
  data: [],
  loading: false,
  error: null,
  ...
  fetchData: async (year = null) => {
    ...
    try {
      ...
      const response = await api.get(`/gaps/expense/getFilteredGaps?${params}`, {
...
getTotalPages: () => {
  ...
  deleteSelectedItems: async () => {
    ...
    try {
      await api.delete('/gaps/expense/deleteExpenseGaps', {
        headers: { Authorization: `Bearer ${token}` },
...
export default useExpenseGapsStore;